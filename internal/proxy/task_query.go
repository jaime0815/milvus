...
func (t *queryTask) PreExecute(ctx context.Context) error {
	...
	schema, _ := globalMetaCache.GetCollectionSchema(ctx, GetCurDatabaseFromContextOrEmpty(ctx), collectionName)
	t.schema = schema
...
func (t *queryTask) Execute(ctx context.Context) error {
	...
	executeQuery := func() error {
		shards, err := globalMetaCache.GetShards(ctx, true, GetCurDatabaseFromContextOrEmpty(ctx), t.collectionName)
		if err != nil {
			return err
		}
...
func (t *queryTask) PostExecute(ctx context.Context) error {
	...
	schema, err := globalMetaCache.GetCollectionSchema(ctx, GetCurDatabaseFromContextOrEmpty(ctx), t.request.CollectionName)
	if err != nil {
		return err
	}
