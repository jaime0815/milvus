...
func (g *getStatisticsTask) PreExecute(ctx context.Context) error {
	...
	collID, err := globalMetaCache.GetCollectionID(ctx, GetCurDatabaseFromContextOrEmpty(ctx), g.collectionName)
...
func (g *getStatisticsTask) getStatisticsFromQueryNode(ctx context.Context) error {
	...
	executeGetStatistics := func(withCache bool) error {
		shard2Leaders, err := globalMetaCache.GetShards(ctx, withCache, GetCurDatabaseFromContextOrEmpty(ctx), g.collectionName)
		if err != nil {
			return err
		}
	...
	if errors.Is(err, errInvalidShardLeaders) || funcutil.IsGrpcErr(err) || errors.Is(err, grpcclient.ErrConnect) {
		...
		globalMetaCache.DeprecateShardCache(GetCurDatabaseFromContextOrEmpty(ctx), g.collectionName)
...
func checkFullLoaded(ctx context.Context, qc types.QueryCoord, collectionName string, searchPartitionIDs []UniqueID) ([]UniqueID, []UniqueID, error) {
	...
	info, err := globalMetaCache.GetCollectionInfo(ctx, GetCurDatabaseFromContextOrEmpty(ctx), collectionName)
	if err != nil {
...
func (g *getPartitionStatisticsTask) Execute(ctx context.Context) error {
	...
	g.collectionID = collID
	partitionID, err := globalMetaCache.GetPartitionID(ctx, GetCurDatabaseFromContextOrEmpty(ctx), g.CollectionName, g.PartitionName)
	if err != nil {
		return err
	}
	req := &datapb.GetPartitionStatisticsRequest{
