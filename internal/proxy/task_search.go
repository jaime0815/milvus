...
func getPartitionIDs(ctx context.Context, collectionName string, partitionNames []string) (partitionIDs []UniqueID, err error) {
	...
	partitionsMap, err := globalMetaCache.GetPartitions(ctx, GetCurDatabaseFromContextOrDefault(ctx), collectionName)
	if err != nil {
		return nil, err
	}
...
func checkIfLoaded(ctx context.Context, qc types.QueryCoord, collectionName string, searchPartitionIDs []UniqueID) (bool, error) {
	info, err := globalMetaCache.GetCollectionInfo(ctx, GetCurDatabaseFromContextOrDefault(ctx), collectionName)
	if err != nil {
