...
func getPartitionIDs(ctx context.Context, collectionName string, partitionNames []string) (partitionIDs []UniqueID, err error) {
	...
	partitionsMap, err := globalMetaCache.GetPartitions(ctx, GetCurDatabaseFromContextOrEmpty(ctx), collectionName)
	if err != nil {
		return nil, err
	}
...
func (t *searchTask) PreExecute(ctx context.Context) error {
	...
	t.schema, _ = globalMetaCache.GetCollectionSchema(ctx, GetCurDatabaseFromContextOrEmpty(ctx), collectionName)
...
func (t *searchTask) Execute(ctx context.Context) error {
	...
	executeSearch := func() error {
		shard2Leaders, err := globalMetaCache.GetShards(ctx, true, GetCurDatabaseFromContextOrEmpty(ctx), t.collectionName)
		if err != nil {
			return err
		}
	...
	err := retry.Do(retryCtx, func() error {
		...
		if searchErr != nil {
			...
			globalMetaCache.DeprecateShardCache(GetCurDatabaseFromContextOrEmpty(ctx), t.collectionName)
		}
		return searchErr
