...
func (cit *createIndexTask) getIndexedField(ctx context.Context) (*schemapb.FieldSchema, error) {
	schema, err := globalMetaCache.GetCollectionSchema(ctx, GetCurDatabaseFromContextOrEmpty(ctx), cit.req.GetCollectionName())
	if err != nil {
...
func (dit *describeIndexTask) PreExecute(ctx context.Context) error {
	...
	collID, err := globalMetaCache.GetCollectionID(ctx, GetCurDatabaseFromContextOrEmpty(ctx), dit.CollectionName)
	if err != nil {
		return err
	}
...
func (dit *describeIndexTask) Execute(ctx context.Context) error {
	schema, err := globalMetaCache.GetCollectionSchema(ctx, GetCurDatabaseFromContextOrEmpty(ctx), dit.GetCollectionName())
	if err != nil {
		log.Error("failed to get collection schema", zap.Error(err))
...
func (gist *getIndexStateTask) Execute(ctx context.Context) error {
	collectionID, err := globalMetaCache.GetCollectionID(ctx, GetCurDatabaseFromContextOrEmpty(ctx), gist.CollectionName)
	if err != nil {
		return err
	}
