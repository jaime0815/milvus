...
func (dt *deleteTask) getChannels() ([]pChan, error) {
	...
	collID, err := globalMetaCache.GetCollectionID(dt.ctx, GetCurDatabaseFromContextOrEmpty(dt.ctx), dt.CollectionName)
	if err != nil {
		return nil, err
	}
...
func (dt *deleteTask) PreExecute(ctx context.Context) error {
	...
	if len(dt.PartitionName) > 0 {
		...
		partID, err := globalMetaCache.GetPartitionID(ctx, GetCurDatabaseFromContextOrEmpty(ctx), collName, partName)
		if err != nil {
	...
	schema, err := globalMetaCache.GetCollectionSchema(ctx, GetCurDatabaseFromContextOrEmpty(ctx), collName)
	if err != nil {
