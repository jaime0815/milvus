...
func validateCollectionNameOrAlias(entity, entityType string) error {
	...
	if int64(len(entity)) > Params.ProxyCfg.MaxNameLength {
		return fmt.Errorf("%s the length of a collection %s must be less than %d characters", invalidMsg, entityType,
			Params.ProxyCfg.MaxNameLength)
	}
...
func ValidateResourceGroupName(entity string) error {
	...
	if int64(len(entity)) > Params.ProxyCfg.MaxNameLength {
		return fmt.Errorf("%s the length of a resource group name must be less than %d characters",
...
func GetCurDatabaseFromContext(ctx context.Context) (string, error) {
	md, ok := metadata.FromIncomingContext(ctx)
	if !ok {
		return "", fmt.Errorf("fail to get md from the context")
	}
	header := md[strings.ToLower(util.HeaderDatabase)]
	if len(header) < 1 {
		return "", nil
	}
	return header[0], nil
